...
def _price_slip_multiplier(price: float, mode: str = "INDIAN") -> float:
    """Price/volatility-adaptive slippage multiplier (v5.22).

    Only INDIAN small-caps (fade universe: MANAKSTEEL, AVROIND, IDEA ...) and
    low-priced altcoins trade on wide relative spreads — a flat 0.05% is
    optimistic for them and overstates paper P&L. US large-caps/ETFs
    (SPY, XLK, QQQ) are mega-liquid; widening their slippage by price would
    be wrong, so US always stays at 1.0x. Tiers:
      price < 20   -> 3.0x  (penny / micro caps, widest spread)
      price < 100  -> 2.0x  (small caps)
      price < 500  -> 1.5x  (mid caps)
      else         -> 1.0x  (large caps / liquid)
    """
...
def check_entry_allowed(ticker: str, direction: str,
    ...
    if tf in ("GAP_DOWN_1m", "INTRADAY_1h", "FADE_1h", "US_FADE_5m", "LONG_BOUNCE_5m"):
...
def generate_portfolio_report(current_prices: dict = None):
    ...
    total_closed = closed_cnt if closed_cnt and closed_cnt > 0 else wins + losses
...
<style>
  *, *::before, *::after {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .badge-fade {{ background: #2e3d00; color: #b3ff00; }}
...
<body>
<div class="container">
  <h1>📊 Portfolio Report — v5.17</h1>
  <div class="subtitle">{_html_escape(date_str)} {_html_escape(time_str)} • Generated by FREE 3-Market Paper Trade Bot (SWING + INTRADAY + FADE) • <a href="https://github.com/thokfoot/free-4-market-master" style="color:#58a6ff">thokfoot/free-4-market-master</a></div>
...
''')
    ...
    if os.path.exists(PAPER_FILE):
        ...
        if len(df) > 0:
            ...
            cols = ["Date", "Ticker", "Direction", "TimeFrame", "Entry_Price", "Qty", "SL", "Target", 
                    "Exit_Price", "P&L", "P&L_%", "Status", "Reason"]
            cols = [c for c in cols if c in df.columns]
            _has_tf_col = "TimeFrame" in df.columns
            ...
            for _, row in df.iloc[::-1].iterrows():
                ...
                tf_badge_html = {"FADE_1h": '<span class="badge badge-fade">FD</span>',
                                 ...
                                 "SWING_1d": '<span class="badge badge-swing">SW</span>'}.get(tf_col, _html_escape(tf_col))
                ...
                if _has_tf_col:
                    parts.append(f"<td>{tf_badge_html}</td>")
    ...
    parts.append(f'''
  <div class="footer">
    FREE 3-Market v5.17 Paper Trade Bot &bull;
    ...
    for idx, row in df.iterrows():
        ...
        if _is_india_intraday and now.time() >= datetime.strptime("15:30", "%H:%M").time():
            ...
            if minutes_held >= trade_max_hold:
                ...
                is_expired = True
        elif trade_tf in ("INTRADAY_1h", "FADE_1h", "US_FADE_5m", "LONG_BOUNCE_5m"):
        ...
        if exit_price:
            ...
            _entry_dt_full = IST.localize(datetime.strptime(row["Date"], "%Y-%m-%d"))
            try:
                _et = str(row.get("Time_IST", ""))[:8]
                if len(_et) == 8:
                    _entry_dt_full = IST.localize(datetime.strptime(f"{row['Date']} {_et}", "%Y-%m-%d %H:%M:%S"))
            except Exception:
                pass
            ...
            trade_tf = str(row.get("TimeFrame", "SWING_1d"))
            if trade_tf == "US_FADE_5m":
                capital_key = "US_FADE"
            elif trade_tf == "LONG_BOUNCE_5m":
                capital_key = "LONG_BOUNCE"
            elif trade_tf in ("INTRADAY_1h", "FADE_1h", "GAP_DOWN_1m"):
                capital_key = "INTRADAY"
            else:
